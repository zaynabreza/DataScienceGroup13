...
def get_Questions(filePath, llm):
    ...
    prompt_template = """
        Identify and list all potential questions from the following text, including those that may might be part of multiple-choice setups:\n\n{text}
    """
    ...
    for page in docs:
        prompt = prompt_template.format(text=page)
        response = llm(prompt)  # Assuming a method like this exists
        # This should return a list of questions or similar structure
        questions.extend(response)  # Adjust based on the actual model's output format

    # Return structured JSON
    return json.dumps({"questions": questions}, indent=4)
